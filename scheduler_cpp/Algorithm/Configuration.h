...
public:
	...
	inline string GetDayById(int id)
	{
		map<int, string>::iterator it = _days.find( id );
		return it != _days.end() ? ( *it ).second : nullptr;
	}

	inline string GetTimeById(int id)
	{
		map<int, string>::iterator it = _times.find( id );
		return it != _times.end() ? ( *it ).second : nullptr;
	}
