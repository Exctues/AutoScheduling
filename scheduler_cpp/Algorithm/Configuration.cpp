...
void Configuration::ParseFile(char* fileName)
{
	...
	_days.insert(pair<int, string>( 0, "Monday" ));
	_days.insert(pair<int, string>( 1, "Tuesday" ));
	_days.insert(pair<int, string>( 2, "Wednesday" ));
	_days.insert(pair<int, string>( 3, "Thursday" ));
	_days.insert(pair<int, string>( 4, "Friday" ));
	_days.insert(pair<int, string>( 5, "Saturday" ));
	_days.insert(pair<int, string>( 6, "Sunday" ));

	_times.insert(pair<int, string>( 0, "09:00-10:30" ));
	_times.insert(pair<int, string>( 1, "10:35-12:05" ));
	_times.insert(pair<int, string>( 2, "12:10-13:40" ));
	_times.insert(pair<int, string>( 3, "14:10-15:40" ));
	_times.insert(pair<int, string>( 4, "15:45-17:15" ));
	_times.insert(pair<int, string>( 5, "17:20-18:50" ));
	_times.insert(pair<int, string>( 6, "18:55-20:25" ));
	_times.insert(pair<int, string>( 7, "20:30-22:00" ));
...
StudentsGroup* Configuration::ParseStudentsGroup(ifstream& file)
{
	...
	while( !file.eof() )
	{
		...
		if( key.compare("id") == 0 )
			...
			number = (int) (atoi( value.c_str() ) * 0.9);
	}
...
CourseClass* Configuration::ParseCourseClass(ifstream& file)
{
	...
	while( !file.eof() )
	{
		...
		if( key.compare("professor") == 0 )
			...
			lab = value.compare( "true" ) == 0;
		else if( key.compare("tutorial") == 0 )
			tutorial = value.compare( "true" ) == 0;
	...
	// make object and return pointer to it
	CourseClass* cc = new CourseClass( p, c, groups, lab, tutorial, dur );
	return cc;
}
