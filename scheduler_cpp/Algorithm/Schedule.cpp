				   ...
				   _fitness(0)
{
	...
	_criteria.resize( Configuration::GetInstance().GetNumberOfCourseClasses() * 4 );
}
...
Schedule::Schedule(const Schedule& c, bool setupOnly)
{
	...
	else
	{
		...
		// reserve space for flags of class requirements
		_criteria.resize( Configuration::GetInstance().GetNumberOfCourseClasses() * CRITERIA_NUMBER );
	}
...
void Schedule::CalculateFitness()
{
	...
	for( map<CourseClass*, int>::const_iterator it = _classes.begin(); it != _classes.end(); ++it, ci += CRITERIA_NUMBER )
	{
        ...
        string course = (*it).first->GetCourse().GetName();
        int lesson_type_number = (*it).first->GetLessonTypeNumber();
        int rooms_number = Configuration::GetInstance().GetNumberOfRooms();
       int day_id_1 = p / rooms_number / DAY_HOURS;
        p -= day_id_1 * rooms_number * DAY_HOURS;
        int room_id_1 = p / DAY_HOURS;
        int time_id_1 = p - room_id_1 * DAY_HOURS;
        for( auto it2 = _classes.begin(); it2 != _classes.end(); ++it2 )
        {
            int id2 = (*it).second;
            int day_id_2 = id2 / rooms_number / DAY_HOURS;
            id2 -= day_id_2 * rooms_number * DAY_HOURS;
            int room_id_2 = id2 / DAY_HOURS;
            int time_id_2 = id2 - room_id_2 * DAY_HOURS;
            if (day_id_2 > day_id_1 || day_id_2 == day_id_1 && time_id_2 > time_id_1) {
                continue;
            }
            if ((*it2).first->GetCourse().GetName() == course && (*it2).first->GetLessonTypeNumber() > lesson_type_number) {
              wrong_order = true;
            }
        }
        ...
        _criteria[ ci + 1 ] = !wrong_order;


		int day_id = p / rooms_number / DAY_HOURS;
		p -= day_id * rooms_number * DAY_HOURS;
		int room_id = p / DAY_HOURS;
		int time_id = p - room_id * DAY_HOURS;

		// check that current time slot is not the last one
		_criteria[ ci + 2 ] = time_id + 1 < DAY_HOURS;
		if (_criteria[ ci + 2 ]) {
			score++;
		}
		...
		_criteria[ ci + 3 ] = r->GetNumberOfSeats() >= cc->GetNumberOfSeats();
		if( _criteria[ ci + 3 ] )
			score++;
...
total_overlap:
	...
	_fitness = (float)score / ( Configuration::GetInstance().GetNumberOfCourseClasses() * CRITERIA_NUMBER );
}
