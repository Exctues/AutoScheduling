...
private:
	...
	// Indicates whether class requires computers
	bool _requiresLab;

	// Indicates whether class is of tutorial type
	bool _isTutorial;
...
public:
	...
	CourseClass(Professor* professor, Course* course, const list<StudentsGroup*>& groups,
		bool requiresLab, bool isTutorial, int duration);
	...
	// Returns TRUE if class is of tutorial type.
	inline bool IsTutorial() const { return _isTutorial; }

	// Returns duration of class in hours
	inline int GetDuration() const { return _duration; }

	int GetLessonTypeNumber();
