...
class GeneticSchedule:
    ...
    @staticmethod
    def run(initial: Population, uni_conf_, calculate_ = True, max_execution_time = 5 * 60) -> Schedule:
        ...
        while population.best_individual.fitness > config['good_error']:
            # terminate if generation takes too long (reserve one minute for Google Spreadsheet building and other)
            if time() - start > min(max_execution_time, 90) - 60:
               break
