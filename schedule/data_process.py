...
class DataProcessor:
    ...
    def get_uniconf(data: dict) -> dict:
        ...
        for group in data['student_groups']:
            group = group[0]
            for super_group in data['student_groups']:
                super_group = super_group[0]
                if group.startswith(super_group):
                    if not (super_group in uni_conf['subgroups']):
                        uni_conf['subgroups'][super_group] = [group]
                    else:
                        uni_conf['subgroups'][super_group].append(group)
    ...
    def enumerate_data(data: dict) -> (dict, dict):
        ...
        for group in data['student_groups']:
            ...
            if groups.get(group) is None:
                groups[group] = i
                DataProcessor.denumeration_data['student_groups'].append(group)
                i += 1
